...
create table if not exists public.driver_vehicles (
  id bigint generated by default as identity primary key,
  "driverId" bigint not null references public.drivers (id) on delete cascade,
  model text not null,
  plate text not null,
  nickname text,
  ...
  "createdAt" timestamptz not null default now(),
  "updatedAt" timestamptz not null default now(),
  constraint driver_vehicles_plate_unique unique (plate)
);
...
alter table public.deliveries
  add column if not exists "originPostalCode" text;

alter table public.deliveries
  add column if not exists "destinationPostalCode" text;
...
create index if not exists idx_driver_vehicles_driver_id on public.driver_vehicles ("driverId");
create index if not exists idx_driver_vehicles_is_primary on public.driver_vehicles ("driverId", "isPrimary");
create index if not exists idx_deliveries_route_order on public.deliveries ("driverId", "scheduledAt", "routeOrder");
...
drop policy if exists "driver_vehicles_select_authenticated" on public.driver_vehicles;
create policy "driver_vehicles_select_authenticated"
on public.driver_vehicles
for select
to authenticated
using (true);

drop policy if exists "driver_vehicles_write_authenticated" on public.driver_vehicles;
create policy "driver_vehicles_write_authenticated"
on public.driver_vehicles
for all
to authenticated
using (true)
